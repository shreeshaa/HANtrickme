...
# clf_seq.load_state_dict_from_url('https://drive.google.com/file/d/1VMO7GoAPn69zw0OLSW0PDjbsww-pMayU/view?usp=sharing',map_location='cpu')
...
#         corpus.append(gosscontent)
#     for gosscom in gosscoms:
#         corpus.append(gosscom)
...
# pickle.dump(gossip_ids, open("gossip_ids.pkl", "wb"))
# pickle.dump(gosscontents, open("gosscontents.pkl", "wb"))
# pickle.dump(gosscoms, open("gosscoms.pkl", "wb"))
# pickle.dump(data_dic, open("data_dic.pkl", "wb"))


# pickle.dump(corpus, open("corpus.pkl", "wb"))
# pickle.dump(reference, open("reference.pkl", "wb"))
# pickle.dump(ldas, open("ldas.pkl", "wb"))
...
def home(username):
    user = username
    if 'userlist' in session:
        if user != 'favicon.ico':
            session['userlist'].append(user)
    else:
        if user != 'favicon.ico':
...
def predict():
    ...
    org_comments = [data_dic[id]['comments']]
    ...
    bleue = (one_gram+two_gram+three_gram)/3
    ...
    for index, lda in enumerate(ldas):
        ...
        # print(cosine(lda.transform(generated_texts_in_feature_vectors.reshape(1, -1)),lda.transform(target_article_in_feature_vectors.reshape(1, -1))))
    one_relevancy_scoree= 1 - (one_relevancy_scoree / (len(ldas)))
    
    # print(org_comments)
    comment = org_comments.copy()
    # print(org_comments)
    # print(comment)
    comment.append(new_comm)
    # print(org_comments)
    # print(comment)
    ...
    with open("D:\home\site\repository\text.txt", "a") as myfile:
...
#     print("\n")
    ...
    return render_template('v_timestamp_new.html', prediction_text=prediction_text[session['user']], art = art, id=id, comments = org_comments, 
